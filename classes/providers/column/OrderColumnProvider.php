...
class OrderColumnProvider
{
    ...
    private function get_field()
    {
        ...
        $output['field'] .= '<td data-item-id="' . esc_attr($this->order['id']) . '" data-editable="' . $editable . '" data-item-title="#' . esc_attr($this->order['id']) . '" data-col-title="' . esc_attr($this->column_data['title']) . '" data-field="' . esc_attr($this->column_key) . '" data-field-type="' . esc_attr($this->field_type) . '" style="' . esc_attr($color['background']) . ' ' . esc_attr($color['text']) . '"';
    ...
    private function image_field()
    {
        ...
        $image = (!empty($image)) ? $image : esc_html__('No Image', WOBEF_NAME);
    ...
    private function checkbox_dual_model_field()
    {
        ...
        return "<label><input type='checkbox' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "' data-item-id='" . esc_attr($this->order['id']) . "' value='yes' class='wobef-dual-mode-checkbox wobef-inline-edit-action' " . esc_attr($checked) . "><span>" . esc_html__('Yes', WOBEF_NAME) . "</span></label>";
    }
    ...
    private function customer_field()
    {
        ...
        return "<a href='javascript:;'  class='wobef-td160 wobef-customer-details' {$modal} data-item-id='" . esc_attr($this->order['id']) . "' data-item-name='" . esc_Attr($this->order[$this->column_key]) . "' data-field='" . esc_attr($this->column_key) . "' data-field-type='" . esc_attr($this->field_type) . "'>" . sanitize_text_field($this->order[$this->column_key]) . "</a>";
    }
}
