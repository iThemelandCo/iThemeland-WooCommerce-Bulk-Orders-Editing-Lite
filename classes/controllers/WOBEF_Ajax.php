...
class WOBEF_Ajax
{
    ...
    private function __construct()
    {
        ...
        add_action('wp_ajax_wobef_inline_edit', [$this, 'inline_edit']);
        ...
        add_action('wp_ajax_wobef_edit_by_calculator', [$this, 'edit_by_calculator']);
        ...
        add_action('wp_ajax_wobef_orders_bulk_edit', [$this, 'orders_bulk_edit']);
    ...
    public function inline_edit()
    {
        if (isset($_POST)) {
            $result = false;
            if (!is_array($_POST['orders_ids'])) {
                return false;
            }

            // sanitize data
            $order_ids = array_map('intval', $_POST['orders_ids']);
            ...
            $this->save_history($order_ids, $field_for_history, sanitize_text_field($_POST['value']), History::INLINE_OPERATION);

            $result = $this->order_repository->update($order_ids, [
            ...
            if ($result) {
                ...
                $this->make_response([
                    'success' => true,
                    'message' => esc_html__('Success !', WOBEF_NAME),
            ...
            if (is_array($_POST['order_ids'])) {
                ...
                if (!empty($order_ids)) {
                    foreach ($order_ids as $order_id) {
                        ...
                        $order = $this->order_repository->get_order(intval(esc_sql($order_id)));
                        if (!($order instanceof \WC_Order)) {
                            return false;
                        }
                        if (is_array($field) && isset($field[1])) {
                            ...
                            switch ($field[0]) {
                                case 'custom_field':
                                    $price = (isset($order_fields['custom_field'][$field[1]][0])) ? floatval($order_fields['custom_field'][$field[1]][0]) : 0;
                                    break;
                            }
                            ...
                            $regular_price = 0; //floatval($order->get_regular_price());
                            $sale_price = 0; //floatval($order->get_sale_price());
                            $order_fields = $this->order_repository->order_to_array($order);
                            $price = $order_fields[$field];
                        ...
                        if (!empty($round_item)) {
                            $value = \wobef\classes\helpers\Order_Helper::round($value, $round_item);
                        }

                        $this->save_history($order_ids, $field_for_history, $value, History::INLINE_OPERATION);

                        $result = $this->order_repository->update([$order_id], [
                            'field_type' => $field_type,
                            'field' => $field_name,
                            'value' => $value,
                            'operator' => null,
                ...
                $histories_rendered = Render::html(WOBEF_VIEWS_DIR . 'history/history_items.php', compact('histories'));
                $edited_ids = $order_ids;
                $edited_ids[] = intval($_POST['order_id']);
                if ($result) {
                    $this->make_response([
                        'success' => true,
                        'message' => esc_html__('Success !', WOBEF_NAME),
    ...
    public function orders_bulk_edit()
    {
        if (!empty($_POST['new_data']) && is_array($_POST['new_data'])) {
            if (!empty($_POST['order_ids'])) {
                $order_ids = array_map('intval', $_POST['order_ids']);
            } elseif (!empty($_POST['filter_data'])) {
                $args = \wobef\classes\helpers\Order_Helper::set_filter_data_items(Sanitizer::array($_POST['filter_data']), [
                    'posts_per_page' => '-1',
                    'fields' => 'ids',
                    'post_status' => 'any',
            ...
            $this->save_history($order_ids, $fields, $new_value, History::BULK_OPERATION);
            ...
            foreach ($new_data as $field => $data_item) {
                if (!empty($data_item)) {
                    if ($field == 'custom_field') {
                        foreach ($data_item as $item) {
                            if (!empty($item['value'])) {
                                $this->order_repository->update($order_ids, [
                                    'field_type' => $field,
                                    'field' => $item['field'],
                                    'value' => $item['value'],
    ...
    public function delete_orders()
    {
        if (isset($_POST['order_ids']) && is_array($_POST['order_ids']) && !empty($_POST['delete_type'])) {
            $orders_ids = array_map('intval', $_POST['order_ids']);
            switch ($_POST['delete_type']) {
                case 'trash':
                    foreach ($orders_ids as $order_id) {
                        $this->save_history_for_delete($order_id);
                        wp_trash_post(intval(esc_sql($order_id)));
                    }
                    break;
                case 'permanently':
                    foreach ($orders_ids as $order_id) {
                        wp_delete_post(intval(esc_sql($order_id)));
                    }
                    break;
            }
            ...
            $this->make_response([
                ...
                'history_items' => $histories_rendered,
                'reverted' => !empty($reverted),
                'edited_ids' => $orders_ids,
            ]);
        }
        return false;
    }

    private function save_history_for_delete($order_id)
    {
    ...
    {
        $message = esc_html__('Error !', WOBEF_NAME);
